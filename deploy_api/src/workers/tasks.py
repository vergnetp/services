        ...
        async with get_db_connection() as conn:
            ...
            for droplet in droplets:
                ...
                try:
                    ...
                    try:
                        health_response = await agent_client.health()
                        ...
                        # Check containers
                        containers = await agent_client.list_containers()
                        
                        for container in containers:
                            container_name = container.get("name", container.get("Names", ["unknown"])[0])
                            container_status = container.get("status", "unknown").lower()
                            
                            results["containers_checked"] += 1
                            
                            # Determine health status
                            if "up" in container_status or container_status == "running":
                                status = "healthy"
                                results["healthy"] += 1
                            elif "exited" in container_status or container_status == "dead":
                                status = "unhealthy"
                                results["unhealthy"] += 1
                            else:
                                status = "degraded"
                            ...
                            await health_store.record(
                                ...
                                error_message=None if status == "healthy" else f"Container status: {container_status}",
                            )
