...
class HealthCheckStore:
    ...
    async def count_recent_healing_attempts(
        self,
        droplet_id: str,
        container_name: str = None,
        action_type: str = None,
    ) -> int:
        """Count healing attempts since last healthy status.
        
        Counts records where action_taken is set (non-null) since the last
        healthy check. This naturally resets when the resource becomes healthy.
        
        Args:
            droplet_id: The droplet ID
            container_name: Container name (None for droplet-level actions)
            action_type: Optional filter by action value 
                         ("restarted", "restart_failed", "rebooted", "reboot_failed")
                         If None, counts all actions.
        
        Returns:
            Number of healing actions taken since last healthy check.
        """
        if container_name:
            where = "[droplet_id] = ? AND [container_name] = ?"
            params = [droplet_id, container_name]
        ...
        recent = await self._crud.list(
            ...
            params=tuple(params),
            order_by="[checked_at] DESC",
            limit=50,  # Look back further for healing attempts
        )
        
        count = 0
        for check in recent:
            # Stop counting at first healthy check (reset point)
            if check["status"] == "healthy":
                break
            
            # Count actions taken
            action = check.get("action_taken")
            if action:
                if action_type is None or action == action_type:
                    count += 1
