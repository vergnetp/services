...
IMPORTANT: Database is managed by app_kernel. Use:
    - from shared_libs.backend.app_kernel.db import db_connection  (for routes)
    - from shared_libs.backend.app_kernel.db import get_db_connection      (for workers)
    - from shared_libs.backend.app_kernel.db import get_db_manager          (for health checks)
"""
...
# =============================================================================
from shared_libs.backend.app_kernel.db import (
...
from shared_libs.backend.databases.connections import AsyncConnection
...
# AI Agents
# =============================================================================
from shared_libs.backend.ai.ai_agents import (
    Agent,
    AgentDefinition,
    tool,
    GuardrailError,
...
from shared_libs.backend.ai.ai_agents.store import (
...
from shared_libs.backend.ai.ai_agents.core import Message, ToolCall, ProviderRateLimitError
from shared_libs.backend.ai.ai_agents.costs import CostTracker, PROVIDER_COSTS, BudgetExceededError
from shared_libs.backend.ai.ai_agents.security import SecurityAuditLog, SecurityEvent
from shared_libs.backend.ai.ai_agents.testing import run_security_audit, AuditReport
from shared_libs.backend.ai.ai_agents.guardrails import ContentGuardrail, InjectionGuard
from shared_libs.backend.ai.ai_agents.providers import CascadingProvider

# =============================================================================
# Attachments
# =============================================================================
from shared_libs.backend.attachments import (
    Attachment,
    AttachmentStore,
...
# RAG Pipeline
# =============================================================================
from shared_libs.backend.ai.documents import DocumentStore
from shared_libs.backend.ai.vectordb import MemoryStore, Document

# =============================================================================
# Configuration
# =============================================================================
from shared_libs.backend.config.base_config import BaseConfig
...
def _warmup_providers(config):
    ...
    from shared_libs.backend.ai.ai_agents.providers import get_provider
...
def preload_ai_models():
    ...
    try:
        from shared_libs.backend.ai.documents import preload_models
        preload_models()
    except ImportError:
        pass
...
def get_document_store(require_models: bool = False) -> DocumentStore:
    ...
    if _document_store is None:
        ...
        from shared_libs.backend.ai.documents import get_model_status, wait_for_models, get_embedder, get_reranker
        logger = get_logger()
...
def get_ai_models_status() -> dict:
    ...
    """
    try:
        from shared_libs.backend.ai.documents import get_model_status
