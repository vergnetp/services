...
The HTTP module includes connection pooling for efficient reuse of HTTP connections across requests.

### Automatic Pooling (Recommended)

When using cloud clients (DOClient, CloudflareClient, StripeClient), connection pooling is automatic:
...
async with AsyncDOClient(api_token="xxx") as client:
    ...
    # TCP connection reused for subsequent calls
```
...
# Before first request, configure limits
configure_pool_limits(PoolLimits(
    max_connections=100,     # Max concurrent connections per base_url
    max_keepalive=20,        # Idle connections to keep warm
    keepalive_expiry=60.0,   # Seconds before closing idle connection
))

# Or use presets
configure_pool_limits(PoolLimits.high_concurrency())  # For LLM streaming
configure_pool_limits(PoolLimits.low_traffic())       # For scripts
```

### FastAPI Lifecycle Integration

If using `app_kernel.create_service()`, connection pools are automatically closed on shutdown.

For manual FastAPI apps:
...
| Decorators | Method | Args | Returns | Category | Description |
|------------|--------|------|---------|----------|-------------|
| `@classmethod` | `default` | | `PoolLimits` | Factory | Standard settings (100 max, 20 keepalive) |
| `@classmethod` | `high_concurrency` | | `PoolLimits` | Factory | High-traffic settings (200 max, 50 keepalive) |
| `@classmethod` | `low_traffic` | | `PoolLimits` | Factory | Script/low-traffic settings (20 max, 5 keepalive) |
