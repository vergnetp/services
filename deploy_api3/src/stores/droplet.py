...
class DropletStore(WorkspaceScopedStore):
    ...
    async def list_all_active(
        ...
        """List all active droplets across all workspaces (for health checks)."""
        return await self.list(
            db,
            where_clause="[status] = ? AND [deleted_at] IS NULL",
            params=("active",),
    ...
    async def get_for_workspace(
        ...
        """Get droplet if it belongs to workspace."""
        droplet = await self.get(db, droplet_id)
        if droplet and droplet.get("workspace_id") == workspace_id:
            return droplet
        return None
